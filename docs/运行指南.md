...
推荐（前端联调）：使用固定端口 `18080` + `docker` profile（与前端默认代理一致）：
```bash
bash backend/run_backend_18080.sh
```

最小启动（仅健康检查，默认端口 `8080`）：
```bash
cd backend
mvn test
mvn spring-boot:run
...
收集视频脚本：
```bash
# ⚠️ 不要在仓库中硬编码第三方 API Key；请使用环境变量注入。
# 说明见脚本顶部注释：scripts/crawl_and_import.sh

# pixabay 示例：
export PIXABAY_API_KEY="YOUR_PIXABAY_API_KEY"
export ADMIN_PASS="YOUR_ADMIN_PASS"
./scripts/crawl_and_import.sh --source pixabay --query "funny" --count 20 --admin-pass "${ADMIN_PASS}"

# pexels 示例：
export PEXELS_API_KEY="YOUR_PEXELS_API_KEY"
./scripts/crawl_and_import.sh --source pexels --query "funny" --count 20 --admin-pass "${ADMIN_PASS}"
```
...
1) **端口冲突**
- MySQL：3307（容器 3306），Redis：6379，MinIO：9000/9001，Kafka：9093，HDFS：9870/8020/9864
- 修改 `deploy/.env` 后重新 `docker compose up -d`

2) **前端 Vite 代理报错：connect ECONNREFUSED 127.0.0.1:18080**
- 原因：前端 dev server 会将 `/api/*` 代理到后端；如果后端未在 `18080` 启动（例如你只运行了默认 `8080` 的 `mvn spring-boot:run`），就会连接被拒绝
- 处理：
  - 推荐：按本文档用 `bash backend/run_backend_18080.sh` 启动后端（`docker` profile + `18080`）
  - 或者：为前端指定代理目标（启动命令前设置环境变量）
    - bash（WSL/Linux/macOS）：`VITE_PROXY_TARGET=http://localhost:8080 npm run dev`
    - PowerShell（Windows）：`$env:VITE_PROXY_TARGET="http://localhost:8080"; npm run dev`

3) **Kafka 连接不上（advertised listeners）**
...
5) **MySQL 连接报错：Public Key Retrieval is not allowed**
